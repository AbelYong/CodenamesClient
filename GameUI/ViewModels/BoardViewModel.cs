...
namespace CodenamesClient.GameUI.ViewModels
{
    public class BoardViewModel : INotifyPropertyChanged
    {
        ...
        private void HandleAgentPicked(object sender, int newTurnLength)
        {
            Application.Current.Dispatcher.Invoke(() =>
            {
                if (_amISpymaster)
        ...
        private void HandleAssassinPicked(object sender, string finalTime)
        {
            Application.Current.Dispatcher.Invoke(() =>
            {
                StopTimer();
        ...
        {
            if (string.IsNullOrWhiteSpace(ChatInput) || !IsChatEnabled) return;
        ...
        public void HandleAgentSelection()
        {
            AddTime(TurnLength);
            MatchOperation.Instance.NotifyPickedAgent(TurnTimer);
        }

        public void HandleBystanderSelection()
        {
            StopTimer();
            TurnTimer = 0;
            if (TimerTokens > 0)
            {
                TimerTokens--;
                MatchOperation.Instance.NotifyPickedBystander(TokenType.TIMER, TimerTokens);
            }
            else
            {
                MatchOperation.Instance.NotifyPickedBystander(TokenType.TIMER, 0);
            }
        ...
        public void HandleAssassinSelection()
        {
            StopTimer();
            MatchOperation.Instance.NotifyPickedAssassin();
        }
